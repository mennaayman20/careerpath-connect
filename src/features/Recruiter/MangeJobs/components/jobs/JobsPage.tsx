...
import { JobCard } from "./JobsCard"; // تأكد من مسار الاستيراد الصحيح
...
const PaginationBtn: React.FC<{
  ...
  <button
    disabled={disabled}
    onClick={onClick}
    className={`px-4 py-2 rounded-xl text-[13px] font-syne font-semibold border transition-all duration-200
      ${active
        ? "bg-[#2D236A] text-white border-[#2D236A] shadow-md"
        : disabled
        ? "bg-white text-slate-300 border-slate-100 cursor-not-allowed"
        : "bg-white text-slate-500 border-slate-200 hover:border-[#2D236A]/30 hover:text-[#2D236A] cursor-pointer"
      }`}
  >
    {label}
  </button>
    ...
    <div className="min-h-screen bg-[#f6f5ff]">
      ...
      >
        ...
        <main className="max-w-screen-xl mx-auto px-6 py-8">
          ...
          {/* Jobs List */}
{/* Jobs List - هنا يتم عرض القائمة بالكامل */}
          {loading ? (
...
<footer className="mt-5 bg-white rounded-2xl border border-[#2D236A]/8 px-6 py-4 flex items-center gap-6 flex-wrap shadow-sm">
  <span className="font-syne text-[10px] font-bold uppercase text-[#1ca37b]">💡 Recruiter Tips</span>

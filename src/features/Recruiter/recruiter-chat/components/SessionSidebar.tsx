        ...
        {isLoading ? (
          ...
          sessions.map((session) => (
            <div
              ...
              className={`session-item ${activeSessionId === session.sessionId ? 'session-item--active' : ''} ${isStreaming ? 'session-item--disabled' : ''}`}
              onClick={() => {
                if (!isStreaming) {
                  onSelect(session.sessionId);
                }
              }}
              title={isStreaming ? 'Please wait for the AI to finish' : ''}
              style={{
                cursor: isStreaming ? 'not-allowed' : 'pointer',
                opacity: isStreaming && activeSessionId !== session.sessionId ? 0.6 : 1
              }}
            >
              ...
              <button
                ...
                onClick={(e) => { 
                  e.stopPropagation(); 
                  if (!isStreaming) onDelete(session.sessionId); 
                }}
                title={isStreaming ? 'Cannot delete while streaming' : 'Delete session'}
                style={{
                  cursor: isStreaming ? 'not-allowed' : 'pointer',

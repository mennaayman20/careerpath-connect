...
export const RecruiterChatPage: React.FC<RecruiterChatPageProps> = ({ defaultJobId }) => {
  ...
  const [prefillJobId, setPrefillJobId] = useState<number | undefined>(defaultJobId);
  ...
  // Auto-open modal pre-filled with the job from the drawer
  useEffect(() => {
    if (defaultJobId) setIsModalOpen(true);
  }, [defaultJobId]);

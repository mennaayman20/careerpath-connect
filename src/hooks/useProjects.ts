...
export const useProject = () => {
  ...
  // 1. جلب المشاريع (Fetching)
  // دي بتفضل هنا عشان هي اللي بتغذي الصفحة كلها بالبيانات
  const { data: projects = [], isLoading } = useQuery({
    queryKey: ["profile", "projects"],
    queryFn: userProfileService.getUserProjects,
    staleTime: 5 * 60 * 1000, // الداتا تفضل Fresh لمدة 5 دقائق
  });

  // 2. إضافة مشروع جديد (Create)
  // بنخليها هنا لأنها بتضيف عنصر "جديد" للقائمة الكبيرة
  const addMutation = useMutation({
    mutationFn: () => 
      userProfileService.addUserProject({
        title: "New Project",
    ...
    onSuccess: () => {
      // بنعمل Invalidate عشان الـ Query اللي فوق (projects) تعيد جلب الداتا بالـ ID الجديد
      queryClient.invalidateQueries({ queryKey: ["profile", "projects"] });
      toast({ title: "Added", description: "New project created successfully." });
    },
    onError: () => {
      toast({ 
        variant: "destructive", 
  ...
  // منطق التحقق من صحة البيانات (عشان زرار الـ "Add" يتفعل أو يتلغي)
  ...
  return { 
    ...
    isLoading,                // حالة تحميل القائمة لأول مرة
    addProject: addMutation.mutate, // وظيفة إضافة مشروع
    isAdding: addMutation.isPending, // حالة تحميل زرار "Add Project" فقط
    isInvalidate,             // هل فيه مشاريع ناقصة بيانات؟
  };
};
